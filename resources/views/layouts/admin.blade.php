<!DOCTYPE html>
<html lang="es" class="{{ auth()->check() && auth()->user()->theme === 'dark' ? 'dark' : '' }}">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>@yield('title') | ControlPro Delivery</title>
            ...
            <aside
               ...
               flex flex-col items-center lg:items-stretch py-8 transition-all duration-300">
                ...
                <div class="px-6 mb-12 flex items-center gap-3">
                    <div class="w-9 h-9 bg-orange-500 rounded-xl flex items-center justify-center text-white">
                    ...
                    <span class="hidden lg:block font-bold text-xl tracking-tight">
                        {{ auth()->user()->empresa->nombre ?? 'Mi Empresa' }}
               ...
               {{ request()->routeIs('dashboard') ? 'bg-orange-100 text-orange-500' : 'hover:bg-orange-100 hover:text-orange-500' }}">
                    ...
                    <a href="{{ route('paquetes.index') }}"
                        class="flex items-center gap-4 px-4 py-3 rounded-xl transition
               {{ request()->routeIs('paquetes.*') ? 'bg-orange-100 text-orange-500' : 'hover:bg-orange-100 hover:text-orange-500' }}">
                        <span class="material-icons-outlined">inventory_2</span>
                        <span class="hidden lg:block">Paquetes</span>
                    </a>

                    <a href="{{ route('motoristas.index') }}"
                        class="flex items-center gap-4 px-4 py-3 rounded-xl transition
               {{ request()->routeIs('motoristas.*') ? 'bg-orange-100 text-orange-500' : 'hover:bg-orange-100 hover:text-orange-500' }}">
                        <span class="material-icons-outlined">emoji_people</span>
                        <span class="hidden lg:block">Motoristas</span>
                    </a>

                    <a href="{{ route('clientes.index') }}"
                        class="flex items-center gap-4 px-4 py-3 rounded-xl transition
               {{ request()->routeIs('clientes.*') ? 'bg-orange-100 text-orange-500' : 'hover:bg-orange-100 hover:text-orange-500' }}">
                        <span class="material-icons-outlined">people</span>
                        <span class="hidden lg:block">Clientes</span>
                    </a>
                ...
                <div class="px-4 mt-auto">
                    <div class="mt-6 flex items-center gap-3 px-2">
                        ...
                        <div class="hidden lg:block overflow-hidden">
                            ...
                            <p class="text-xs text-slate-500 truncate">{{ auth()->user()->empresa->nombre ?? '' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <header
                    ...
                    <div class="flex items-center gap-6">
                        ...
                        <form method="POST" action="{{ route('cambiar.tema') }}">
                            ...
                            <button type="submit"
                                class="w-10 h-10 rounded-xl flex items-center justify-center hover:bg-orange-100 transition">
